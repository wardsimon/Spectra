	...
	)
{
	...
	double nw,s,r1,r2; 	/* for double Lorentzian */
...
/*	mexPrintf("%f %f %f \n",dvx,dvy,dvz); */
	...
	*/
	...
	/* now cross-section part! -Double Lorentzian. */
        /* Physica B, 234-236 (1997) 1107             */
	/* define here your signal expression s = ... */

	nw=1./(exp(hkle[3]*bf/d[12])-1);	/* Bose temperature factor.*/
	r1=hkle[3]-wq;		/* difference of energies */
        r2=hkle[3]+wq;         /* sum of energies  */   
					/* normalized Double Lorentzian */
	s=(nw+1)/(pi*wq)*(d[11]/(r1*r1+d[11]*d[11])-d[11]/(r2*r2+d[11]*d[11]));
	return s;
}   
