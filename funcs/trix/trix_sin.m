function [y, name, pnames, pin]=trix_sin(x,p, flag)
% trix      : 4D convoluted fit function for TAS
% MFIT function [y, name, pnames, pin]=trix_sin(x,p, flag)
% for triple-axis data. A sinusiodal S(Q,w) is convoluted with the
% 4D resolution function, and then a Gaussian peak is
% added to simulate an incoherent peak.
% The form of the dispersion is:
% w^2=((A-E0)*sin(P*pi*q))^2-E0^2');
%       A= energy at Brillouin zone boundary
%		  E0= energy at Brillouin zone centre
%       P= periodicity
%       e.g. for Bragg peaks at (200), (400), etc. P=0.5 
%
% Calls to: rescal, trixsqw, rc_re2rc, rc_cnmat, rc_savp, mc_monte
%   and mcint_sin.c (must compile this with the matlab mex function)
%	
% Originally Des McMorrow 25.1.96
% Modified ARW 26.3.02
...
%      At the moment, only works for meV!
   ...
   f=0.4826;
   mon_flag=p(17);               %  mon_flag: 1=monitor, 0=time
...
%----- Passing relevant parameters to mcint  

    NMC=p(16);
    for j=1:lx
	pres(31:34)=pscan(1:4)+(x(j)-xstart)/(xend-xstart)*dscan(1:4);

	partemp=[ p(14) p(15) p(4) p(5) p(6) p(7) p(8) p(9) p(10) p(11) p(12) p(13) ];
	[smcmex]=mcint_sin(NMC,pres(31:34),...
...
%----- Add incoherent peak
   ...
   y=p(1)*y+p(2)+p(18)*exp(-0.5*(x/p(19)).^2);
...
%----- Mfit parameter window
	...
	pnames=str2mat('Amplitude','Background','Energy offset',...
					'BZC H','BZC K','BZC L',...
					'Period H','Period K','Period L',...
					'Energy BZB H','Energy BZB K','Energy BZB L',...
					'Gap (meV)','Width',...
					'Temperature','NMC','Mon Flag',...
                    'AMP_i_n_c','sigma_i_n_c');

	if flag==1, pin=[20 0 0 0 2 0 0.5 0.5 0.5 10 10 10 0 0.005 4.2 200 1 1 0.001]; else pin = p; end
	if flag==2
		mf_msg('Sorry, this function does not support guess.')
		disp('TRIX v1 4D-TAS function')
		disp('Simple dispersion of type w^2=((A-E0)*sin(P*pi*q))^2-E0^2');
		disp('A= energy at Brillouin zone boundary');
		disp('E0= energy at Brillouin zone centre');
      disp('P= periodicity');
      disp('e.g. for Bragg peaks at (200), (400), etc. P=0.5')
      disp(' ');
   end
