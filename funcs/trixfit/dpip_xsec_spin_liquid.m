...
%        p(12)   = Jr, rung exchange parameter
...
s=0;
E_cutoff=0.01*Jl;
E_exc=disp_ladder_sc(Qh,[Jr 0 Jl]);
...
for j=1:length(n_w)
        if w(j)> E_cutoff
            ...
            s=s+(mff.^2).*(n_w(j)+1).*sign(w(j)).*(1-0.5*cos(2*pi*(h_.*a1(1)+k_.*a1(2)+l_.*a1(3)))-0.5*cos(2*pi*(h_.*a2(1)+k_.*a2(2)+l_.*a2(3))))./(sqrt(2*pi)*sigma).*exp(-0.5.*((w(j)-E_exc(j))/sigma).^2)./w(j);
%            s=s+(mff.^2).*(n_w(j)+1).*sign(w(j)).*(1-0.5*cos(2*pi*(h_.*a1(1)+k_.*a1(2)+l_.*a1(3)))-0.5*cos(2*pi*(h_.*a2(1)+k_.*a2(2)+l_.*a2(3))))./(sqrt(2*pi)*sigma).*exp(-0.5.*((w(j)-E_exc(j))/sigma).^2);
        end
    end
