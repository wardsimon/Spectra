function [s]=dpip_xsec_notest(NMC,Qvec,p,fwhm,b_mat,load_the_matrices,long_trans_flag)
%
...
if load_the_matrices==1
     ...
     if m == 0.125
             ...
             grid_caux=0.25.*smp0d125+0.25.*spm0d125;
             disp('loaded m=0.125 transverse part of cross-section')
     end
     if m == 0.25
            ...
            grid_caux=0.25.*smp0d25+0.25.*spm0d25;
            disp('loaded m=0.25 transverse part of cross-section')
     end
     if m == 0.375
             ...
             grid_caux=0.25.*(smp0d375+spm0d375);
             disp('loaded m=0.375 transverse part of cross-section')
     end
     if m == 0.5
             ...
             grid_caux=0.5*smp0d5;
             disp('loaded m=0.5 transverse part of cross-section');
     end
 end
...
%            s=s+(n_w(j)+1).*sign(w(j)).*(grid_caux(ie(j),ik(j)));
 ...
 %  figure
%     clf
%     pcolor(kk,ee,grid_caux(1:length(ee),:))
%     shading flat
%     caxis([0 20]);
%     hold on
     for j=1:length(n_w)
