...
if length(p)< (28+offset)
%    warning('rc_popma:ParameterError','The parameter file is missing monitor information. This file was generated in rescal v5') 
   monitorw=3;
   monitorh=3;
   L1mon = 3*L1/4;
else
...
% check for incident guide
if flag_guide == 1,
    alf0_guide=pit*2*pi*guide_h/ki;
    bet0=pit*2*pi*guide_v/ki;
    if (alf0_guide <= alf0)
        alf0=alf0_guide;
    end;  %take into account collimator in the guide
end
G=zeros(8,8);
...
% MI1= B*A*(inv(inv(D*(inv(S+T'*F*T))*D')+G))*A'*B'; % including spatial effects.
MI= B*A/((inv(D*(inv(S+T'*F*T))*D')+G))*A'*B'; % including spatial effects.

% MI=B*A*(inv(G+C'*F*C))*A'*B';  % Cooper and Nathans matrix,
MI(2,2)=MI(2,2)+q0^2*etas^2;
MI(3,3)=MI(3,3)+q0^2*etasp^2;
M=inv(MI);
NP=5.545*M;                        % Correction factor as input parameters, 5.545=8*log(2)
N=NP;                
...
% R0=P0p/(64*pi^2*sin(thetam)*sin(thetaa))*sqrt(det(S)*det(F)/det(S+T'*F*T))/det(NP)*(2*pi)^2;
% 
...
sqrt(det(F)/det((D*(S+T'*F*T)^(-1)*D')^(-1)+G)); %Popovici

% % Werner and Pynn correction
% % for mosaic spread of crystal.
% % (this assumes zero mosaicity out-of plane)
% %R0=abs(R0/(etas*sqrt(1/etas^2+q0^2*N(2,2)/5.545)));  
R0=abs(R0/(sqrt(1+(q0*etas)^2*N(2,2)/5.545))/(sqrt(1+(q0*etasp)^2*N(3,3)/5.545)));  
% %test=R0;
...
% %----- Final error check
% 
% %plot(q0,R0,'bo')%,q0,det(RM_),'ro',q0,det(F),'go',q0,1/det((D*(S+T'*F*T)^(-1)*D')^(-1)+G),'ko');
% %plot(q0,det(RM_),'bo',q0,det(inv(D*(S+T'*F*T)D')+G)),'ro')
% %plot(q0,det(F)/det((D*(S+T'*F*T)^(-1)*D')^(-1)+G)) % changing weakly
% %plot(q0,Rm*Ra*(2*pi)^4/(64*pi^2*sin(thetam)*sin(thetaa)),'ro') % not changing
% %plot(q0,abs(R0/(etas*sqrt(1/etas^2+q0^2*N(2,2)))),'ro',q0,abs(R0/(sqrt(1+(q0*etas)^2*N(2,2)))/(sqrt(1+(q0*etas)^2*N(3,3)))),'bo')
% %plot(q0,MI(2,2),'ro',q0,MI(3,3),'bo'); %changing, depending on etas
% %plot(q0,Rmon,'ro'); % not changing over our scan
% %plot(q0,Rm,'bo',q0,Ra,'ro') % not changing
% %plot(q0,B(1,1),'bo',q0,B(1,2),'ro')
% % after having checked that thetaa, thetam change only slightly, phi is
% % found to change by 7 %, so B is changing during the scan
% %plot(q0,phi/pi*180,'ro',q0,2*thetas/pi*180,'bo',q0,(thetas+pi/2)/pi*180,'ko')
% 
% hold on
