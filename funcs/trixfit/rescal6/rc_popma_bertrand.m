...
%                                 If mon_flag=0,    "          "          "     time
%
% Output:       R0= resoltuion volume calculated using Dorner's method (obsolete).
% The calculation of the resolution volume R0 is taken from Zheludev' Reslib library 
% and Bruno Dorner's formula has been removed as it worked only with
% collimators; the correction for monitor efficiency has also been included
% BR 2004
...
%		are inclucded as yet in this program) is also set
%		to zero.
...
L1mon=p(28+offset);	%distance monochromator monitor (cm)
monitorw=p(29+offset)./sqrt(12.);	%monitor width (cm)
monitorh=p(30+offset)./sqrt(12.);	%monitor height (cm)
...
if flag_guide == 1, alf0_guide=pit*2*pi*guide_h/ki; bet0=pit*2*pi*guide_v/ki; end 
if (alf0_guide <= alf0) alf0=alf0_guide; end;  %take into account collimator in the guide
...
R0=abs(R0/(etas*sqrt(1/etas^2+q0^2*N(2,2))));  
