...
initialise.monte_carlo_samples=120000;        % Monte Carlo steps for convolution integration    
...
pin=[pin 0.2 0.5 0 90 0.284];   %----- Cross-sec parameters defined in xsec_file
pin=[pin 200 0 0 1];            %----- Background parameters used in bkgd_file
...
fcp=[0.05 100 0.000001];notfixed=zeros(size(pin));
%fcp=[0.01 niter 0.0001];notfixed=zeros(size(pin));
notfixed([13])=1;
[s,f]=fits(s,'trixfit',pin,notfixed,fcp)
np=100;
sigma=0.005;
xmin=min(getfield(s,'x'))-0.2
xmax=max(getfield(s,'x'))+0.2;
