...
if nargin==2
    if length(p)==18 % the case when start and end points of a scan are given, together with the number of points in the scan
...
%          R=[cos_theta sin_theta 0; -sin_theta cos_theta 0; 0 0 1]*[cos(ph/180*pi) sin(ph/180*pi) 0; -sin(ph/180*pi) cos(ph/180*pi) 0;0 0 1];
    ...
    %----- Evaluate the cross-section and do the Monte Carlo integration
        ...
        s = feval(xsec,NMC,[Qs(1)+t*dQ(1) Qs(2)+t*dQ(2) Qs(3)+t*dQ(3) Qs(4)+t*dQ(4)],p,global_fwhm(j,1:4),global_b_mat(j,1:16),load_the_matrices);
        load_the_matrices=0;
