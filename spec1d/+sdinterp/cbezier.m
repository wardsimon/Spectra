...
u = [x(1) + 2*x(2); arrayfun(@(x,y) 4*x + 2*y,x(2:end-2),x(3:end-1)); (8*x(n-1) + x(n))/2.0]';
v = [y(1) + 2*y(2); arrayfun(@(x,y) 4*x + 2*y,y(2:end-2),y(3:end-1)); (8*y(n-1) + y(n))/2.0]';
...
%    decomp = @(l) recur(@(f, k, b) iff(k == 1, u(1)/b, true,...
%                     iif(k < n-1, @() u(k) - f(f, k-1,4.0-1/b), ...
%        true,                     @() u(k) - f(f, k-1,3.5-1/b))),l);
   for i = 2:n-1
      tmp(i) = 1/b;
      if (i < n-1)
         b = 4.0 - tmp(i);
