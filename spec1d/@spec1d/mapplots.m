function [xi,yi,zi]=mapplot(s,extvar,leer,smoothe,xi,yi,scale)
%
...
for n=1:length(s)
   ...
   if size(extvar)==[length(getfield(s(n),'x')) length(s)]
      ...
      if 1==1
          xp=[xp;extvar(:,n)];
          yp=[yp;getfield(s(n),'x')];
      end
   else
      x=[x;extvar(n)*ones(size(getfield(s(n),'x')))];  
      if 1==1
          xp=[xp;extvar(n)*ones(size(getfield(s(n),'x')))];
          yp=[yp;getfield(s(n),'x')];
      end
  end
end
...
if nargout==0
  pcolor(xi,yi,zi)
  shading flat
 % line(xp,yp,'linestyle','none','marker','.','color','k')  
end
