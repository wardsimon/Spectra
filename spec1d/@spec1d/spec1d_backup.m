...
% Simon Ward 25/01/2016
    ...
    if nargin == 0
        ...
        p.addParamValue('yfit',[],@(x) validateattributes(x,{'numeric','gpuArray'},{'vector','real','finite','nonnan'},mfilename,'yfit'))
        p.addParamValue('x_label',[],@(x) validateattributes(x,{'char'},{},mfilename,'x_label'))
        p.addParamValue('y_label',[],@(x) validateattributes(x,{'char'},{},mfilename,'y_label'))
        p.addParamValue('datafile',[],@(x) validateattributes(x,{'char'},{},mfilename,'datafile'))
    ...
    for i = 1:length(s_in)
        ...
        if (length(s_in(i).x) > sdext.getpref('minGPULength').val) && sdext.getpref('gpuArray').val && checkGpuMemory(s_in(i).x)
