...
verbose = f_in.fitdata.Display; %This will decide if to tell them the results
dFdp    = f_in.fitdata.JacobFcn;

stol  = f_in.fitdata.TolGradCon;
...
if nargin > 2
    if ~isempty(options)
        ...
        f_in = f_in.setFitdata('Bounds', bounds);
        f_in = f_in.setFitdata('TolCon', conf_p);
    else
        conf_p = f_in.fitdata.TolCon;
        bounds(:,1) =  f_in.fitdata.Bounds(:,1);
        bounds(:,2) =  f_in.fitdata.Bounds(:,2);
    end
end
...
if (cvg ~= 1)
    ...
    if ((n1 > 10) && (n2 > 10)) % sufficent data for test?
        ...
        if (zed < 0)
            ...
            warning('MATLAB:poorFit','COVERGENCE NOT ACHIEVED!\n%0.03f%% chance of fewer than %i runs. Try changing fcp...',prob,nrun);
        else
            prob = erfc(zed/sqrt(2))/2*100;
            warning('MATLAB:poorFit','COVERGENCE NOT ACHIEVED!\n%0.03f%% chance of greater than %i runs. Try changing fcp...',prob,nrun);
        end
    end
end
